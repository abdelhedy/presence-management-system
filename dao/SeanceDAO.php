...
class SeanceDAO
{
    ...
    {
        $query = "SELECT s.*, c.nom_cours, c.code_cours, c.id_enseignant
                  FROM " . $this->table . " s
    ...
    public function findByCours($idCours)
    {
        $query = "SELECT s.*,
                  ...
                  FROM " . $this->table . " s
                  LEFT JOIN inscriptions i ON s.id_cours = i.id_cours AND i.statut = 'inscrit'
    ...
    /**
     * Récupérer toutes les séances du jour pour un étudiant
     */
